...
int main(int argc, char* argv[]) {
  ...
  const std::string model_path =
      argc == 4 ? argv[1] : "/tmp/mobilenet_v1_1.0_224_quant_edgetpu.tflite";
  const std::string resized_image_path = argc == 4 ? argv[2] : "/tmp/resized_cat.bmp";
  const std::string label_path = argc == 4 ? argv[3] : "/tmp/imagenet_labels.txt";
  ...
  const std::vector<uint8_t>& input =
      coral::read_bmp(resized_image_path, &width, &height, &channels);
